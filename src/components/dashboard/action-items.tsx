...
export function ActionItems({ items }: { items: ActionItem[] }) {
  ...
  return (
    <section className="rounded-xl bg-amber-50/50 p-6">
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((item) => (
          <Link key={item.type} href={item.href}>
            <div
              className={`flex items-center gap-3 rounded-lg border border-l-4 bg-white p-3.5 shadow-sm transition-all hover:shadow-md ${COLOR_MAP[item.type]}`}
            >
              {ICON_MAP[item.type]}
